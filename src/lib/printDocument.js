...
body{font-size:10.5pt;line-height:1.45}.document{width:100%;max-width:182mm;margin:0 auto;padding-bottom:14mm}
.document-header{padding-bottom:12px;border-bottom:2px solid var(--ink);margin-bottom:12px}.brand-row{display:flex;justify-content:space-between;align-items:flex-start;gap:24px}.brand{display:flex;align-items:center;gap:10px}.brand-logo{width:128px;height:auto;max-height:22mm;object-fit:contain;object-position:left center}.brand-name-fallback{font-size:15px;font-weight:800;letter-spacing:-.02em}.brand-sub{font-size:8.5pt;color:var(--muted);margin-top:2px}.document-meta{text-align:right;color:var(--muted);font-size:7.8pt;line-height:1.5}.document-meta strong{display:block;color:var(--ink);font-size:8.2pt;letter-spacing:.05em}.document-title{margin-top:14px}.document-title h1{margin:0;font-size:21pt;line-height:1.12;letter-spacing:-.035em;font-weight:800}.document-title p{margin:4px 0 0;color:var(--muted);font-size:9pt}
...
.signature-area{display:grid;grid-template-columns:1fr 1fr;gap:34px;margin-top:42px;break-inside:avoid;page-break-inside:avoid}.signature{padding-top:23px;border-top:1px solid #667085;text-align:center;font-size:8.5pt;color:#344054}.footer{display:none}.print-footer{display:flex;align-items:center;justify-content:space-between;gap:12px;position:fixed;left:0;right:0;bottom:-14mm;height:11mm;padding:2mm 14mm 0;border-top:1px solid var(--line);background:#fff;color:#98a2b3;font-size:7pt;z-index:20}.print-footer img{width:62px;height:auto;max-height:8mm;object-fit:contain}.page-break{break-before:page;page-break-before:always}.keep-together{break-inside:avoid;page-break-inside:avoid}.no-print{display:none!important}
@media screen{body{background:#eef1f5;padding:24px}.document{background:#fff;max-width:210mm;min-height:297mm;padding:18mm;box-shadow:0 8px 30px rgba(16,24,40,.10)}.print-footer{position:static;margin-top:24px}.print-footer img{width:72px}}
@media print{body{background:#fff}.document{max-width:none;padding-bottom:12mm}.no-print{display:none!important}}
`;
...
export function brandHeader(company="ProdOS", title="Documento", meta=[]){
  ...
  return `<header class="document-header"><div class="brand-row"><div class="brand"><img class="brand-logo" src="/brand/prodpersonal-logo.svg" alt="ProdPersonal" onerror="this.style.display='none';this.nextElementSibling.style.display='block'"/><span class="brand-name-fallback" style="display:none">ProdPersonal</span><div><div class="brand-sub">Gestão de Pessoas • Ecossistema ProdOS</div></div></div>${fields?`<div class="document-meta">${fields}</div>`:""}</div><div class="document-title"><h1>${esc(safeTitle)}</h1></div></header>`;
}

function buildDocument({title="Documento",subtitle="",company="ProdOS",content="",confidential=true,documentCode=""}={}){
 const now=new Date().toLocaleString("pt-BR");
 const companyName=typeof company==="string"?company:(company?.name||company?.razao_social||"ProdOS");
 ...
 <header class="document-header"><div class="brand-row"><div class="brand"><img class="brand-logo" src="/brand/prodpersonal-logo.svg" alt="ProdPersonal" onerror="this.style.display='none';this.nextElementSibling.style.display='block'"/><span class="brand-name-fallback" style="display:none">ProdPersonal</span><div><div class="brand-sub">${esc(companyName)} • Gestão de Pessoas</div></div></div><div class="document-meta"><strong>DOCUMENTO CORPORATIVO</strong>${documentCode?`Código: ${esc(documentCode)}<br>`:""}Emissão: ${esc(now)}</div></div><div class="document-title"><h1>${esc(title)}</h1>${subtitle?`<p>${esc(subtitle)}</p>`:""}</div></header>
 ${confidential?'<div class="confidential">CONFIDENCIAL — USO RESTRITO AO RH/DP</div>':""}${content}<footer class="print-footer"><img src="/brand/prodpersonal-logo.svg" alt="ProdPersonal"/><span>${esc(companyName)} • ProdPersonal</span><span>Documento corporativo • ${esc(now)}</span></footer></main><script>window.addEventListener('load',()=>setTimeout(()=>window.print(),350));</script></body></html>`;
}
