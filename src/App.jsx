...
function Guard({ children, allow }) {
  ...
  return allow.includes(role) || role === "master" || role === "admin" ? children : <Navigate to="/dashboard" replace />;
}
function PrivateArea() {
  ...
  const active = !!profile?.platform_role || ["active", "vitalicio"].includes(subscription?.subscription_status);
  const route = (path, element, allow = []) => <Route path={path} element={<Guard allow={allow}>{element}</Guard>} />;
  if (!active) return <Layout><Routes><Route path="/assinatura" element={<AssinaturaPage />} /><Route path="*" element={<PendingSubscriptionPage />} /></Routes></Layout>;
  return <Layout><Routes>
    <Route path="/" element={<Navigate to="/dashboard" replace />} />
    {route("/dashboard", <DashboardPage />, ["employee","gestor","rh","dp","admin","master"])}
    {route("/empresa", <EmpresaPage />, ["employee","gestor","rh","dp","admin","master"])}
    ...
    {route("/treinamentos", <TreinamentosPage />, ["gestor","rh","dp","admin","master"])}
