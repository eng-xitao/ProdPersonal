...
export default function AssinaturaPage() {
  ...
  async function loadPlans() {
    setLoading(true);
    const { data } = await supabase.from("plans").select("*").eq("active", true).eq("product_key", "prodpersonal").order("sort_order");
    setPlans(data ?? []);
    setLoading(false);
  }
  ...
  async function confirmAndSubscribe(plan) {
    ...
    const { data, error } = await supabase.functions.invoke("create-subscription", {
      body: { companyId: profile.company_id, plan: plan.key, productKey: "prodpersonal" },
    });
