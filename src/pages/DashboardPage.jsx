...
function MiniChart({ label, value, caption }) {
  const safe = Math.max(0, Math.min(100, Number(value) || 0));
  return <div style={styles.chartBox}><div style={styles.chartTrack}><div style={{ ...styles.chartBar, height: `${Math.max(8, safe)}%` }}><span style={styles.chartValue}>{Math.round(safe)}%</span></div></div><strong style={styles.chartLabel}>{label}</strong><small style={styles.helper}>{caption}</small></div>;
}

export default function DashboardPage() {
  const { company, profile, session } = useAuth();
  return (profile?.access_role || "employee") === "employee" ? <EmployeePortal company={company} profile={profile} session={session} /> : <ManagementDashboard company={company} />;
}

function EmployeePortal({ company, profile, session }) {
  const [employee, setEmployee] = useState(null);
  const [pending, setPending] = useState([]);
  const [metrics, setMetrics] = useState({ evaluations: 0, evaluationPct: 0, goals: 0, goalPct: 0, trainings: 0, trainingsDone: 0, pdi: 0, pdiPct: 0, feedbacks: 0 });
  ...
  async function loadPortal() {
    ...
    try {
      ...
      const [r, ev, goals, tr, pdi, fb, ci] = await Promise.all([
        supabase.from("hr_communication_recipients").select("id,status,communication_id,hr_communications:communication_id(title,communication_type,due_at,file_name)").eq("employee_id", emp.id).in("status", ["pending", "visualizado", "pendente"]),
        supabase.from("hr_performance_evaluations").select("id,overall_score").eq("employee_id", emp.id),
        supabase.from("hr_goals").select("id,target_value,current_value").eq("employee_id", emp.id),
        ...
        supabase.from("hr_climate_checkins").select("mood").eq("employee_id", emp.id).eq("checkin_date", today).maybeSingle(),
      ]);
      const firstError = [r, ev, goals, tr, pdi, fb, ci].find((x) => x.error)?.error;
      if (firstError) throw firstError;
      const goalRows = goals.data || [];
      const goalRowsScored = goalRows.filter((x) => Number(x.target_value) > 0);
      const goalPct = goalRowsScored.length ? goalRowsScored.reduce((sum, x) => sum + Math.min(100, Number(x.current_value || 0) / Number(x.target_value) * 100), 0) / goalRowsScored.length : goalRows.length ? 50 : 0;
      const trRows = tr.data || [];
      const pdiRows = pdi.data || [];
      const scores = (ev.data || []).map((x) => Number(x.overall_score)).filter(Number.isFinite);
      const evaluationPct = scores.length ? (scores.reduce((a, b) => a + b, 0) / scores.length / 10) * 100 : 0;
      const pdiPct = pdiRows.length ? pdiRows.filter((x) => x.status === "concluido").length / pdiRows.length * 100 : 0;
      setPending(r.data || []);
      setMetrics({ evaluations: (ev.data || []).length, evaluationPct, goals: goalRows.length, goalPct, trainings: trRows.length, trainingsDone: trRows.filter((x) => x.attended).length, pdi: pdiRows.filter((x) => x.status !== "concluido").length, pdiPct, feedbacks: (fb.data || []).length });
      setMood(ci.data?.mood || null);
...
const styles = {
  ...
  portalHero: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 20, marginBottom: 18, flexWrap: "wrap" },
  portalTitle: { fontFamily: "var(--font-display)", fontSize: 28, margin: "5px 0 0" },
  ...
  card: { background: "var(--panel)", border: "1px solid var(--line)", borderRadius: "var(--radius)", padding: "17px 16px", display: "flex", flexDirection: "column", gap: 4 },
  cardValue: { fontFamily: "var(--font-display)", fontSize: 28, fontWeight: 800 },
  cardLabel: { fontSize: 12, fontWeight: 700 },
  helper: { color: "var(--text-dim)", fontSize: 11.5 },
  moodBadge: { background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 999, padding: "8px 13px", fontSize: 12, fontWeight: 700 },
  moodCard: { background: "var(--panel)", border: "1px solid var(--line)", borderRadius: "var(--radius)", padding: 18, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 20, flexWrap: "wrap", marginBottom: 16 },
  moodRow: { display: "flex", gap: 8 },
  moodButton: { display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: "var(--panel-2)", border: "1px solid var(--line)", borderRadius: "var(--radius)", padding: "7px 9px", cursor: "pointer", fontSize: 23 },
  moodSelected: { outline: "2px solid var(--amber)" },
  twoCol: { display: "grid", gridTemplateColumns: "minmax(0,1.2fr) minmax(300px,.8fr)", gap: 16, marginBottom: 16 },
  panel: { background: "var(--panel)", border: "1px solid var(--line)", borderRadius: "var(--radius)", padding: 18, marginBottom: 16 },
  sectionHead: { display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" },
  title2: { fontFamily: "var(--font-display)", fontSize: 15, margin: "0 0 5px" },
  count: { minWidth: 26, height: 26, display: "grid", placeItems: "center", borderRadius: 999, background: "var(--amber)", color: "#fff", fontSize: 11, fontWeight: 800 },
  notice: { display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", padding: "11px 0", borderBottom: "1px solid var(--line)" },
  pending: { color: "var(--amber)", fontSize: 11, fontWeight: 800 },
  progressItem: { marginTop: 15 },
  progressHead: { display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 6 },
  track: { height: 8, borderRadius: 999, background: "var(--panel-2)", overflow: "hidden", border: "1px solid var(--line)" },
  fill: { height: "100%", background: "var(--amber)", borderRadius: 999 },
  chartRow: { display: "grid", gridTemplateColumns: "repeat(4,minmax(90px,1fr))", gap: 14, alignItems: "end", marginTop: 14 },
  chartBox: { minHeight: 170, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", gap: 5 },
  chartTrack: { height: 115, width: 48, display: "flex", alignItems: "flex-end", justifyContent: "center", background: "var(--panel-2)", border: "1px solid var(--line)", borderRadius: "10px 10px 4px 4px", overflow: "hidden" },
  chartBar: { width: "100%", minHeight: 8, background: "var(--amber)", display: "flex", alignItems: "flex-start", justifyContent: "center", borderRadius: "8px 8px 0 0", paddingTop: 4, boxSizing: "border-box" },
  chartValue: { color: "#fff", fontSize: 10, fontWeight: 800 },
  chartLabel: { fontSize: 11.5 },
  quickGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10 },
  quick: { padding: 14, border: "1px solid var(--line)", borderRadius: "var(--radius)", display: "flex", flexDirection: "column", gap: 5 },
