...
function EmployeePortal({ company, profile, session }) {
  ...
  async function loadPortal() {
    ...
    try {
      ...
      setMetrics({ evaluations: (evaluations.data ?? []).length, goals: goalRows.length, trainings: participantRows.length, trainingsDone: participantRows.filter((x) => x.attended).length, pdi: (pdi.data ?? []).filter((x) => x.status !== "concluido").length, feedbacks: (feedbacks.data ?? []).length });
  ...
  return <div>
    <header style={styles.portalHero}>
      <div><span style={styles.eyebrow}>Meu Portal</span><h1 style={styles.portalTitle}>Olá, {employee.full_name.split(" ")[0]} 👋</h1><p style={styles.subtitle}>{employee.role || "Colaborador"} · acompanhe aqui suas pendências e desenvolvimento.</p></div>
