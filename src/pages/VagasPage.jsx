...
export default function VagasPage() {
  ...
  const [vacancyMode, setVacancyMode] = useState("existing");
  const [form, setForm] = useState({ title: "", department: "", description: "", job_description_id: "" });
  const [candidateForm, setCandidateForm] = useState({ full_name: "", email: "", phone: "" });

  async function loadAll() {
    setLoading(true); setError("");
    try {
      const [vRes, cRes, jRes] = await Promise.all([
        supabase.from("hr_vacancies").select("id, title, department, description, status, job_description_id, created_at").order("created_at", { ascending: false }).limit(100),
        ...
        supabase.from("hr_job_descriptions").select("id, title, department, summary, responsibilities").order("title"),
      ]);
      if (vRes.error) throw vRes.error;
      if (cRes.error) throw cRes.error;
      if (jRes.error) throw jRes.error;
      setVacancies(vRes.data ?? []); setAllCandidates(cRes.data ?? []); setJobDescriptions(jRes.data ?? []);
  ...
  function pickJobDescription(jobId) {
    const job = jobDescriptions.find((j) => j.id === jobId);
    if (!job) { setForm((f) => ({ ...f, job_description_id: "" })); return; }
    setForm({
  ...
  async function createVacancy(e) {
    ...
    setSaving(true);
    const { error: insertError } = await supabase.from("hr_vacancies").insert({ company_id: company.id, title: form.title.trim(), department: form.department.trim() || null, description: form.description.trim() || null, job_description_id: form.job_description_id || null });
    if (insertError) setError(insertError.message);
    else { setForm({ title: "", department: "", description: "", job_description_id: "" }); setVacancyMode("existing"); setShowNewVacancy(false); await loadAll(); }
    setSaving(false);
  }
  ...
  return (
    <div style={styles.page}>
      ...
      {showNewVacancy && <Modal title="Abrir nova vaga" onClose={() => setShowNewVacancy(false)}>
        <div style={styles.modeToggle}>
          <button type="button" style={{ ...styles.modeBtn, ...(vacancyMode === "existing" ? styles.modeBtnActive : {}) }} onClick={() => setVacancyMode("existing")}>Usar cargo existente</button>
          <button type="button" style={{ ...styles.modeBtn, ...(vacancyMode === "new" ? styles.modeBtnActive : {}) }} onClick={() => { setVacancyMode("new"); setForm((f) => ({ ...f, job_description_id: "" })); }}>Criar descrição nova</button>
        </div>
        <form onSubmit={createVacancy}>
          {vacancyMode === "existing" && (
            jobDescriptions.length === 0 ? (
              <p style={styles.muted}>Nenhum cargo cadastrado ainda em Descrição de Cargos. Escolha "Criar descrição nova" ou cadastre um cargo primeiro.</p>
            ) : (
              <label style={styles.label}>Cargo cadastrado
                <select style={styles.input} value={form.job_description_id} onChange={e => pickJobDescription(e.target.value)} required>
                  <option value="">Selecione...</option>
                  {jobDescriptions.map(j => <option key={j.id} value={j.id}>{j.title}{j.department ? ` — ${j.department}` : ""}</option>)}
                </select>
              </label>
            )
          )}
          <label style={styles.label}>Cargo<input style={styles.input} value={form.title} onChange={e => setForm({...form,title:e.target.value})} placeholder="Ex.: Analista de PCP" required/></label>
          <label style={styles.label}>Departamento<input style={styles.input} value={form.department} onChange={e => setForm({...form,department:e.target.value})} placeholder="Ex.: Produção"/></label>
          <label style={styles.label}>Descrição<input style={styles.input} value={form.description} onChange={e => setForm({...form,description:e.target.value})} placeholder="Objetivo e principais responsabilidades"/></label>
          <div style={styles.modalActions}><button type="button" style={styles.secondaryBtn} onClick={() => setShowNewVacancy(false)}>Cancelar</button><button style={styles.primaryBtn} disabled={saving}>{saving ? "Salvando..." : "Abrir vaga"}</button></div>
        </form>
      </Modal>}
...
const styles = {
  ...
  modeBtn:{flex:1,background:"var(--panel-2)",border:"1px solid var(--line)",color:"var(--text-dim)",borderRadius:9,padding:"9px 0",fontWeight:700,fontSize:12.5,cursor:"pointer"},
  modeBtnActive:{background:"var(--amber)",color:"#fff",borderColor:"var(--amber)"},input:{background:"var(--panel-2)",border:"1px solid var(--line)",borderRadius:9,padding:"10px 11px",color:"var(--text)",fontSize:13,outline:"none",width:"100%",boxSizing:"border-box"},modalActions:{display:"flex",justifyContent:"flex-end",gap:9,marginTop:18},processSummary:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:20},processSummaryDiv:{background:"var(--panel-2)",borderRadius:10,padding:12},formGrid:{display:"grid",gridTemplateColumns:"1.3fr 1fr 1fr auto",gap:8},modalSection:{fontSize:13,margin:"22px 0 10px"},kanban:{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:10,overflowX:"auto"},kanbanCol:{background:"var(--panel-2)",borderRadius:12,padding:10,minHeight:180},kanbanHead:{display:"flex",justifyContent:"space-between",fontSize:11,fontWeight:800,padding:"3px 2px 9px"},candidateCard:{background:"var(--panel)",border:"1px solid var(--line)",borderRadius:9,padding:10,marginBottom:7,cursor:"pointer"},candidateCardSpan:{display:"block",fontSize:10,color:"var(--text-dim)",marginTop:4,overflow:"hidden",textOverflow:"ellipsis"},kanbanEmpty:{fontSize:10,color:"var(--text-dim)",padding:"25px 5px",textAlign:"center"},profileHeader:{display:"flex",alignItems:"center",gap:13,padding:"5px 0 20px"},avatar:{width:52,height:52,borderRadius:14,background:"var(--panel-2)",display:"grid",placeItems:"center",fontSize:20,fontWeight:900},detailGrid:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12},detailGridDiv:{background:"var(--panel-2)",padding:13,borderRadius:10,display:"flex",flexDirection:"column",gap:7,fontSize:11,color:"var(--text-dim)"},statusSelectWide:{marginTop:5,background:"var(--panel)",border:"1px solid var(--line)",borderRadius:8,padding:8,color:"var(--text)"},timeline:{marginTop:16,padding:15,borderRadius:11,background:"var(--panel-2)",color:"var(--text-dim)",fontSize:12},timelineTitle:{fontWeight:800,color:"var(--text)",marginBottom:5}
};
