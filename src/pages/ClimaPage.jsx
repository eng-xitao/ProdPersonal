...
export default function ClimaPage() {
  ...
  async function loadDashboard() {
    ...
    } catch (err) { setError("Não foi possível carregar o painel de clima: " + (err.message || "erro desconhecido")); }
    finally { setLoading(false); }
  }

  useEffect(() => { loadDashboard(); }, [company?.id, profile?.id, role]);
  const latestByEmployee = useMemo(() => { const map = new Map(); [...checkins].sort((a,b)=>String(b.checkin_date).localeCompare(String(a.checkin_date))).forEach(c=>{if(!map.has(c.employee_id))map.set(c.employee_id,c);}); return map; }, [checkins]);
  const total = employees.length, responded = latestByEmployee.size, pending = Math.max(total-responded,0), participation = total ? Math.round((responded/total)*100) : 0;
  ...
  const recentDays = useMemo(()=>{const days=[];for(let i=6;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);const key=d.toISOString().slice(0,10);const values=checkins.filter(c=>c.checkin_date===key);days.push({label:d.toLocaleDateString("pt-BR",{weekday:"short"}).replace(".",""),value:values.length?values.reduce((a,c)=>a+Number(c.mood||0),0)/values.length:0});}return days;},[checkins]);
  ...
  return <div>
    ...
    {loading?<div style={styles.loading}>Carregando indicadores...</div>:<>
      ...
      <section style={styles.grid}>
        ...
        <Card title="Leitura executiva" subtitle="Indicadores para acompanhamento do RH e gestão"><Insight label="Participação" value={`${participation}%`} helper={pending?`${pending} ainda não responderam`:"Todos já responderam"}/><Insight label="Sentimento predominante" value={moodCounts.slice().sort((a,b)=>b.count-a.count)[0]?.label||"Sem dados"} helper="com base nos registros disponíveis"/><Insight label="Humor médio" value={`${average}/5`} helper="quanto maior, melhor a percepção" last/></Card>
      </section>
